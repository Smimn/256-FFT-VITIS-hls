...
with open("time_domain.csv", "r", newline="") as f:
    r = csv.DictReader(f)
    for row in r:
        n.append(int(row["n"]))
...
with open("spectrum.csv", "r", newline="") as f:
    r = csv.DictReader(f)
    for row in r:
        k.append(int(row["k"]))
...
# ---- Frequency domain (0 .. N-1, log scale) ----
ax3 = fig.add_subplot(gs[1, :])
ax3.plot(k_axis, amp, label="|X[k]| / N")
ax3.set_title("Frequency domain – full range (0 … N−1), one-sided")
